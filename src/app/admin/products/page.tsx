...
export default function AdminProducts() {
    ...
    const [form, setForm] = useState({ name: '', description: '', price: '', stock: '', unit: 'pcs', category: '' });

    const load = () => fetch('/api/products').then(r => r.json()).then(setProducts);
    useEffect(() => { load(); }, []);

    const openAdd = () => { setEditing(null); setForm({ name: '', description: '', price: '', stock: '', unit: 'pcs', category: '' }); setShowForm(true); };
    const openEdit = (p: Product) => { setEditing(p); setForm({ name: p.name, description: p.description || '', price: String(p.price), stock: String(p.stock), unit: p.unit, category: p.category || '' }); setShowForm(true); };
            ...
            <div className="table-wrapper">
                <table>
                    ...
                    <tbody>
                        ...
                        {products.length === 0 && <tr><td colSpan={6}><div className="empty-state">No products found</div></td></tr>}
                    </tbody>
                </table>
            </div>
        </>
